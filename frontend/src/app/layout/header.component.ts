    ...
    <header class="navbar">
      <div class="nav-container">
        ...
        <a routerLink="/menu" class="brand">
          ...
          <span class="brand-name">Pedidos<span class="brand-highlight">360</span></span>
        </a>

        <!-- Desktop Navigation Links -->
        <nav class="nav-links">
          <a routerLink="/menu" routerLinkActive="active" class="nav-link">
            <svg viewBox="0 0 24 24" width="18" height="18" fill="none" stroke="currentColor" stroke-width="2"><path d="M3 3h18v18H3z"/><path d="M3 9h18M9 21V9"/></svg>
            Menú
          </a>
        ...
        <div class="nav-actions">
          ...
          @if (authService.isDemoMode()) {
            <div class="demo-role-selector">
              ...
              >
                <span class="role-icon">{{ authService.isAdmin() ? '🛡️' : '👤' }}</span>
                <span class="role-text">{{ authService.isAdmin() ? 'ADMIN' : 'CLIENTE' }}</span>
              </button>
            </div>
          }
            ...
            <div class="user-menu">
              <div class="user-pill" (click)="toggleDropdown()">
                ...
                <div class="user-info">
                  ...
                  <span class="user-role">{{ authService.isAdmin() ? 'Administrador' : 'Cliente' }}</span>
                </div>
              ...
              @if (isDropdownOpen()) {
                <div class="dropdown-menu">
                  ...
                  <a routerLink="/pedidos" class="dropdown-item" (click)="closeDropdown()">
                    Mis Pedidos
                  </a>
                  @if (authService.isAdmin()) {
          ...
          <button type="button" class="mobile-toggle" (click)="toggleMobileNav()" aria-label="Menú móvil">
      ...
      @if (isMobileNavOpen()) {
        <div class="mobile-nav">
          <a routerLink="/menu" (click)="closeMobileNav()" class="mobile-nav-link">Menú</a>
          @if (authService.isAuthenticated()) {
            <a routerLink="/pedidos" (click)="closeMobileNav()" class="mobile-nav-link">Mis Pedidos</a>
          }
          @if (authService.isAdmin()) {
            <a routerLink="/admin" (click)="closeMobileNav()" class="mobile-nav-link text-warning">Administración</a>
          }
          <a routerLink="/carrito" (click)="closeMobileNav()" class="mobile-nav-link">
            Carrito ({{ cartService.totalCount() }})
          </a>
          @if (authService.isAuthenticated()) {
            <button type="button" class="mobile-nav-link btn-logout" (click)="logout()">Cerrar Sesión</button>
          } @else {
            <a routerLink="/login" (click)="closeMobileNav()" class="mobile-nav-link text-primary">Iniciar Sesión</a>
          }
        </div>
      }
    </header>
