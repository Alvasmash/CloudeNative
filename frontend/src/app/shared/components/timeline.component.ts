    ...
    @if (currentEstado === EstadoPedido.CANCELADO) {
      <div class="cancelled-banner">
        ...
        <div>
          ...
          <p class="cancelled-desc">La orden no continúa en curso. Si requieres asistencia, contacta al soporte.</p>
        </div>
      </div>
    } @else {
      <div class="timeline-wrapper">
        <div class="timeline-steps">
          @for (step of steps; track step.estado; let i = $index) {
            <div class="step-item" [class.completed]="isStepCompleted(step.estado)" [class.active]="currentEstado === step.estado">
              <div class="step-node">
                <div class="node-circle">
                  @if (isStepCompleted(step.estado) && currentEstado !== step.estado) {
                ...
                @if (i < steps.length - 1) {
                  <div class="node-line" [class.filled]="isStepCompleted(steps[i + 1].estado) || currentEstado === steps[i + 1].estado"></div>
                }
              </div>
              <div class="step-info">
